...
def ruku4(function, state, params, dt, steps, noise):
    ...
    state += np.array([n * np.random.randn(len(n)) * np.sqrt(dt)
                       for n in noise]).reshape(state.shape)
...
def euler_maruyama(function, state, params, dt, steps, noise):
    noise = state.shape[1] * [noise] if state.ndim == 2 else [noise]
    for i in range(steps):
        state += dt * function(state, params) + np.array(
            [n * np.random.randn(len(n)) * np.sqrt(dt) for n in noise]).\
            reshape(state.shape)
    return state
