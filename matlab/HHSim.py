...
sim = np.zeros(len(states), TOTAL_TIME)  # to hold simulation results
noise = [noiseStd, np.zeros(len(params), 1)]

for t in range(1, TOTAL_TIME):
    dS = HH_dynamics(states)  # dynamics
    # updated states
    states = states + dS * delta + noise * np.random.randn(size(states))
...
hold on
plot([0 TOTAL_TIME] * delta, Vth * [1 1], '--', 'color', .5 * [1 1 1])
hold off
